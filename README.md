...
| `trading` | Legacy XML fixed-price listing create, revise, relist, and end operations | "Create a live fixed-price listing from this SKU." |
...
| API coverage | 304 tools with broad Sell API coverage, ready to call | Build each request from the docs |
...
A local [Model Context Protocol](https://modelcontextprotocol.io) server that exposes **304 tools** with broad eBay Sell API coverage to AI assistants — inventory, Feed upload tasks, order fulfillment, marketing, analytics, and developer tools.
...
304 tools with broad coverage of eBay's Sell APIs, including general Feed upload-task workflows.
